    ...
    setupHandlers() {
            ...
            try {
                switch (name) {
                    case "get_complete_context":
                        return await this.handleGetCompleteContext(args);
                    case "validate_context_completeness":
                        return await this.handleValidateContext(args);
        ...
        this.server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
            ...
            try {
                ...
                throw new McpError(ErrorCode.InternalError, `Error reading resource ${uri}: ${error instanceof Error ? error.message : String(error)}`);
            }
        });
    }
    async handleGetCompleteContext(args) {
...
`,
    ...
    async handleValidateContext(args) {
    ...
    async handleGetDependencyGraph(args) {
        const graph = await this.analyzer.buildDependencyGraph(args.target, args.projectRoot, args.includeTests || true);
        return {
            content: [
                {
                    type: "text",
