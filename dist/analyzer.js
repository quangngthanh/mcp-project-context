...
export class ProjectAnalyzer {
    ...
    async buildDependencyGraph(target, projectRoot, includeTests = true) {
        ...
        const dependencies = [];
        const dependents = [];
        const collectDependencies = (file, depth = 0) => {
            if (visited.has(file.path) || depth > 5)
    ...
    async getDependencyGraph(projectRoot) {
        const files = Array.from(this.cache.values());
        return {
            nodes: files.map(f => ({
                id: f.relativePath,
    ...
    async getProjectAnalysis(projectRoot) {
        const files = Array.from(this.cache.values());
        return {
    ...
    detectArchitecturalPatterns(files) {
        ...
        if (hasModels)
            patterns.push('Model-Based Architecture');
        if (hasControllers)
    ...
    calculateArchitectureComplexity(files) {
    ...
    analyzeDependencyComplexity(files) {
    ...
    generateProjectSummary(files) {
        ...
        const primaryLanguage = Object.entries(languages).reduce((a, b) => languages[a[0]].count > languages[b[0]].count ? a : b)[0];
    ...
    async reindexFiles(changedFiles) {
    ...
    searchFiles(query) {
        ...
        return files.filter(file => file.relativePath.toLowerCase().includes(queryLower) ||
