export class ContextValidator {
    async validateCompleteness(context, query) {
    ...
    analyzeQuery(query) {
        ...
        analysis.entities = words.filter(word => word.length > 2 &&
            /^[A-Z]/.test(word) ||
            word.includes('.'));
        return analysis;
    }
    validateCodeCompleteness(context, query) {
        const result = {
            score: 0,
            weight: 0.3,
            missingElements: [],
    ...
    validateDependencyCompleteness(context, query) {
        const result = {
            score: 0,
            weight: 0.25,
            missingElements: [],
    ...
    validateStructuralCompleteness(context, query) {
        const result = {
            score: 0,
            weight: 0.2,
            missingElements: [],
    ...
    validateContextCoherence(context, query) {
        const result = {
            score: 0,
            weight: 0.15,
            missingElements: [],
        ...
        const keywordMatches = query.keywords.filter(keyword => contextLower.includes(keyword.toLowerCase()));
        ...
        const entityMatches = query.entities.filter(entity => contextLower.includes(entity.toLowerCase()));
    ...
    validateUsageExamples(context, query) {
        const result = {
            score: 0,
            weight: 0.1,
            missingElements: [],
    ...
    calculateConfidenceScore(completenessScore, context) {
